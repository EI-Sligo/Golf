...
let shotTargetLat = null, shotTargetLng = null; // NEW: Saves target location
...
function getBag() { return sortBag(JSON.parse(localStorage.getItem('castleDarganBag')) || defaultBag); }
function saveBag(bag) { localStorage.setItem('castleDarganBag', JSON.stringify(sortBag(bag))); renderBagUI(); }
...
document.getElementById('add-club-btn').addEventListener('click', () => { let bag = getBag(); let nc = document.getElementById('new-club-select').value; if(!bag.includes(nc)) { bag.push(nc); saveBag(bag); }});
document.getElementById('reset-bag-btn').addEventListener('click', () => saveBag(defaultBag));

const handicapInput = document.getElementById('playing-handicap'); handicapInput.value = localStorage.getItem('castleDarganPlayingHandicap') || 18;
handicapInput.addEventListener('change', (e) => { localStorage.setItem('castleDarganPlayingHandicap', e.target.value); updateScoreInputs(); });
...
handicapSelect.addEventListener('change', (e) => { localStorage.setItem('castleDarganHandicap', e.target.value); recommendClub(currentPlaysLike || parseInt(document.getElementById('distance-number').innerText)); });

function renderShotsList() {
    const shots = JSON.parse(localStorage.getItem('castleDarganShots')) || []; const listEl = document.getElementById('tracked-shots-list');
    ...
    [...shots].reverse().forEach((s, reversedIdx) => {
        const originalIdx = shots.length - 1 - reversedIdx; const dateStr = s.date ? new Date(s.date).toLocaleDateString() : 'Unknown Date';
...
window.deleteShot = function(originalIdx) {
    if (confirm("Delete this shot?")) {
        let shots = JSON.parse(localStorage.getItem('castleDarganShots')) || []; shots.splice(originalIdx, 1); localStorage.setItem('castleDarganShots', JSON.stringify(shots));
        renderShotsList(); updateAnalytics(); if(currentPlaysLike) recommendClub(currentPlaysLike);
    }
}

document.getElementById('export-btn').addEventListener('click', () => {
    const backupData = { shots: localStorage.getItem('castleDarganShots'), scorecard: localStorage.getItem('castleDarganScorecard'), bag: localStorage.getItem('castleDarganBag'), history: localStorage.getItem('castleDarganHistory'), pins: localStorage.getItem('castleDarganPins'), tees: localStorage.getItem('castleDarganTees'), phcap: localStorage.getItem('castleDarganPlayingHandicap') };
...
document.getElementById('import-file').addEventListener('change', (e) => {
    ...
    reader.onload = function(e) {
        try {
            ...
            if (data.shots) localStorage.setItem('castleDarganShots', data.shots); if (data.scorecard) localStorage.setItem('castleDarganScorecard', data.scorecard);
            if (data.bag) localStorage.setItem('castleDarganBag', data.bag); if (data.history) localStorage.setItem('castleDarganHistory', data.history);
            if (data.pins) localStorage.setItem('castleDarganPins', data.pins); if (data.tees) localStorage.setItem('castleDarganTees', data.tees);
...
function initMap() {
    ...
    // 2. RIGHT-CLICK / LONG PRESS HANDLER (Works in Live Round too!)
    map.on('contextmenu', function(e) { saveNewTeeLocation(e.latlng); });

    // 3. LAYUP MARKER (Target for Shot Tracking)
...
function saveNewTeeLocation(latlng) {
    ...
    // Only teleport the user marker if we are in Planner Mode
    if (isPlannerMode) {
...
function updateLayupData(layupLat, layupLng) {
    ...
    const layupRec = getClubRecommendationData(layupPlaysLike); const pinRec = getClubRecommendationData(pinPlaysLike);

    document.getElementById('layup-dist').innerText = distToLayup; document.getElementById('layup-plays').innerText = layupPlaysLike;
    document.getElementById('layup-club').innerText = layupRec ? layupRec.club : 'None';
    document.getElementById('layup-rem').innerText = distToPin; document.getElementById('layup-rem-plays').innerText = pinPlaysLike;
    document.getElementById('layup-rem-club').innerText = pinRec ? pinRec.club : 'None';
}

document.getElementById('clear-layup-btn').addEventListener('click', () => { document.getElementById('layup-info-box').style.display = 'none'; if(layupMarker) { map.removeLayer(layupMarker); layupMarker = null; }});
document.getElementById('recenter-map-btn').addEventListener('click', () => {
    hasCenteredMapThisHole = false; let tLat = courseData[currentHoleIndex].lat, tLng = courseData[currentHoleIndex].lng;
    const customPins = JSON.parse(localStorage.getItem('castleDarganPins')) || {}; if (customPins[currentHoleIndex]) { tLat = customPins[currentHoleIndex].lat; tLng = customPins[currentHoleIndex].lng; }
...
function updateMapState(tLat, tLng) {
    ...
    if (!hasCenteredMapThisHole && currentLat && tLat) { map.fitBounds(L.latLngBounds([[currentLat, currentLng], [tLat, tLng]]), { padding: [20, 20], maxZoom: 18 }); hasCenteredMapThisHole = true; }
}
...
function processLocation(lat, lng) {
    ...
    else { rawYardage = calculateDistance(currentLat, currentLng, tLat, tLng); }
...
function calculateDispersion() {
    ...
    bag.forEach(club => {
        ...
        else if (shots.length < 4) { analytics[club] = { type: 'raw', avg: Math.round(shots.reduce((a,b)=>a+b)/shots.length) }; } 
        else {
            shots.sort((a,b) => a-b); const coreShots = shots.slice(Math.floor(shots.length * 0.20), Math.floor(shots.length * 0.90));
...
document.getElementById('track-btn').addEventListener('click', () => {
    if (!isTrackingShot) {
        ...
        if (layupMarker) {
            shotTargetLat = layupMarker.getLatLng().lat; shotTargetLng = layupMarker.getLatLng().lng;
        } else {
            let tLat = courseData[currentHoleIndex].lat, tLng = courseData[currentHoleIndex].lng;
            const customPins = JSON.parse(localStorage.getItem('castleDarganPins')) || {};
            if (customPins[currentHoleIndex]) { tLat = customPins[currentHoleIndex].lat; tLng = customPins[currentHoleIndex].lng; }
        ...
        if (missDistance <= 15) {
            ...
            if (angleDiff > 5 && angleDiff < 175) {
                autoAccuracy = "Right Rough"; missText = `Missed ${missDistance} yds Right`;
            } 
            // If ball is left of target vector
            else if (angleDiff > 185 && angleDiff < 355) {
                autoAccuracy = "Left Rough"; missText = `Missed ${missDistance} yds Left`;
            } 
...
document.getElementById('save-shot-btn').addEventListener('click', () => {
    ...
    showToast("Shot Saved", () => {
        if(oldShots) localStorage.setItem('castleDarganShots', oldShots); else localStorage.removeItem('castleDarganShots');
...
document.getElementById('reset-pin-btn').addEventListener('click', () => {
    ...
    const hd = courseData[currentHoleIndex]; hasCenteredMapThisHole = false; if(hd.lat) updateMapState(hd.lat, hd.lng);
});
...
document.getElementById('hole-strokes').addEventListener('input', updateScoreInputs); document.getElementById('hole-putts').addEventListener('input', updateScoreInputs);

function updateScoreInputs() {
    const strokes = parseInt(document.getElementById('hole-strokes').value); const putts = parseInt(document.getElementById('hole-putts').value);
    const hd = courseData[currentHoleIndex]; const selectedTee = document.getElementById('tee-box-select').value;
    const activePar = selectedTee === 'red' ? hd.redPar : hd.par; const activeSi = selectedTee === 'red' ? hd.redSi : hd.si;
...
document.getElementById('save-score-btn').addEventListener('click', () => {
    const strokes = parseInt(document.getElementById('hole-strokes').value); if (!strokes) return alert("Enter strokes!");
    const oldScorecard = localStorage.getItem('castleDarganScorecard'); 
    const hd = courseData[currentHoleIndex]; const selectedTee = document.getElementById('tee-box-select').value;
    const activePar = selectedTee === 'red' ? hd.redPar : hd.par; const activeSi = selectedTee === 'red' ? hd.redSi : hd.si;

    let scorecard = JSON.parse(oldScorecard) || {};
    scorecard[currentHoleIndex] = { hole: hd.hole, par: activePar, si: activeSi, strokes: strokes, putts: parseInt(document.getElementById('hole-putts').value) || 0, gir: document.getElementById('hole-gir').checked, points: calculateStableford(strokes, activePar, activeSi) };
    localStorage.setItem('castleDarganScorecard', JSON.stringify(scorecard));
    updateAnalytics(); 
    
    showToast("Hole Score Saved", () => {
        if(oldScorecard) localStorage.setItem('castleDarganScorecard', oldScorecard); else localStorage.removeItem('castleDarganScorecard');
...
function updateAnalytics() {
    ...
    for (let k in scorecard) {
        ...
        totPar += scorecard[k].par; totPts += scorecard[k].points; holesPlayed++; if (scorecard[k].gir) girCount++;
    }
    ...
    if (holesPlayed > 0 && statScoreEl) {
        ...
        statScoreEl.innerText = "E"; document.getElementById('stat-putts').innerText = "0.0";
        document.getElementById('stat-gir').innerText = "0%"; document.getElementById('stat-points').innerText = "0";
    }
    ...
    if (driveCount > 0 && document.getElementById('missTendencyChart')) {
        ...
        missTendencyChartInstance = new Chart(ctxMiss, {
            type: 'doughnut',
            data: { labels: ['Fairway', 'Left', 'Right', 'Short/OB'], datasets: [{ data: [missStats.fairway, missStats.left, missStats.right, missStats.other], backgroundColor: ['#27ae60', '#e74c3c', '#f39c12', '#95a5a6'], borderWidth: 1 }] },
...
document.getElementById('end-round-btn').addEventListener('click', () => {
    ...
    if (confirm("End this round and save to History?")) {
        ...
        localStorage.removeItem('castleDarganScorecard'); updateHoleDisplay(); updateAnalytics();
...
window.deleteRound = function(originalIndex) {
    if (confirm("Permanently delete this round?")) {
        let history = JSON.parse(localStorage.getItem('castleDarganHistory')) || [];
        history.splice(originalIndex, 1); localStorage.setItem('castleDarganHistory', JSON.stringify(history)); renderHistoryTab();
    }
}

function renderHistoryTab() {
    const history = JSON.parse(localStorage.getItem('castleDarganHistory')) || []; const listEl = document.getElementById('past-rounds-list');
    updateAnalytics(); 
